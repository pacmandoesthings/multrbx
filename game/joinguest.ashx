...
// Check for the .ROBLOSECURITY or ROBLOSECURITY cookie
$RBXTICKETO = $_COOKIE['.ROBLOSECURITY'] ?? $_COOKIE['ROBLOSECURITY'] ?? null;
...
switch(true) {
    case ($token !== null):
        ...
        break;
    case ($RBXTICKETO !== null):
        $GetPlayerInfo = $MainDB->prepare("SELECT id, name, membership, admin FROM users WHERE token = :token");
        $GetPlayerInfo->execute([':token' => $RBXTICKETO]);
        $PlayerInfo = $GetPlayerInfo->fetch(PDO::FETCH_ASSOC);
        switch(true) {
            case (!$PlayerInfo):
                die(json_encode(['message' => 'Cannot process your request at this time.']));
        ...
        break;
    default:
        $userId = rand(-20000, -1000);
        $userName = "Guest " . rand(0, 3000);
        $userMembership = "None";
        break;
}
...
switch(true){case(!$ServerInfo):die(json_encode(['message' => 'Cannot process your request at this time.']));break;}
...
nc.ConnectionRejected:connect(function() game:SetMessage("Failed to connect to the game (Connection Rejected)") end)';
	...
	case "json":
		...
		$data = '
{"ClientPort":0,"MachineAddress":"'. $gameIp .'","ServerPort":'. $gamePort .',"PingUrl":"","PingInterval":20,"UserName":"'. $userName .'","SeleniumTestMode":true,"UserId":'. $userId .',"SuperSafeChat":false,"CharacterAppearance":"'. $baseUrl .'/Tools/FetchCharacterAppeareance.aspx?id='. $userId .'","ClientTicket":"","GameId":2,"PlaceId":'. $gameId .',"MeasurementUrl":"","WaitingForCharacterGuid":"26eb3e21-aa80-475b-a777-b43c3ea5f7d2","BaseUrl":"'. $baseUrl .'","ChatStyle":"ClassicAndBubble","VendorId":0,"ScreenShotInfo":"","VideoInfo":"","CreatorId":"'. $gameCreator .'","CreatorTypeEnum":"User","MembershipType":"'. $joinMem .'","AccountAge":3000000,"CookieStoreFirstTimePlayKey":"rbx_evt_ftp","CookieStoreFiveMinutePlayKey":"rbx_evt_fmp","CookieStoreEnabled":true,"IsRobloxPlace":true,"GenerateTeleportJoin":false,"IsUnknownOrUnder13":false,"SessionId":"39412c34-2f9b-436f-b19d-b8db90c2e186|00000000-0000-0000-0000-000000000000|0|190.23.103.228|8|2021-03-03T17:04:47+01:00|0|null|null","DataCenterId":0,"UniverseId":2,"BrowserTrackerId":0,"UsePortraitMode":false,"FollowUserId":0,"characterAppearanceId":'. $userId .'}';
		sign($data);
		break;
	default:
