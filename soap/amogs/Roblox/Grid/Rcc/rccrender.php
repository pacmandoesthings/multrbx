...
class RCCRenderer {
  ...
  function __construct($url = "181.215.226.46", $port = 43640) {
    $this->ip = $url;
    $this->port = $port;
    $this->url = $url.$port;
    $this->SoapClient = new \SoapClient(__DIR__."/RCCService.wsdl", ["location" => "http://".$url.":".$port, "uri" => "http://vrblxxd.ga/", "classmap" => $this->classmap, "exceptions" => false]);
  }
  ...
  function OpenJobEx($job, $script = null) {
    ...
    return RCCRenderer::parseJobResult($result);
  }
