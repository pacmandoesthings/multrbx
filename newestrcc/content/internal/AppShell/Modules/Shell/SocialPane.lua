...
local SocialScreenModule = nil
if not EnableXboxFriendsEvents then
	SocialScreenModule = require(ShellModules:FindFirstChild('SocialScreen'))
end
...
local function CreateSocialPane(parent)
	...
	local onlineFriendsContainer = Utility.Create'Frame'
	{
		...
		Size = UDim2.new(0, EnableXboxFriendsEvents and 1720 or 1438, 0, 610);
		Position = UDim2.new(0, 0, 0, onlineFriendsTitle.Size.Y.Offset);
		BackgroundTransparency = 1;
		Parent = SocialPaneContainer;
	}

	local moreFriendsButton = nil
	if not EnableXboxFriendsEvents then
		moreFriendsButton = Utility.Create'ImageButton'
		{
			Name = "MoreButton";
			BackgroundTransparency = 1;
		...
		local function updateMoreImage(isSelected)
			moreFriendsButton.Image = isSelected and 'rbxasset://textures/ui/Shell/Buttons/MoreButtonSelected@1080.png'
												or  'rbxasset://textures/ui/Shell/Buttons/MoreButton@1080.png'
			moreFriendsButton.Size = UDim2.new(0,100,0,50);
		end
	...
	local friendsScrollingGrid = ScrollingGridModule({Dynamic = EnableXboxFriendsEvents})
