...
function UserThumbnailPlus:SetEmptyThumbnail()
	if self.userThumbnail ~= nil then
		self.userThumbnail.Parent = nil
	end
	self.userThumbnail = Create.new"ImageLabel" {
		Name = "UserThumbnail",
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	self.userThumbnail.Parent = self.rbx
end
...
function UserThumbnailPlus:SetThumbnail(user)
	...
	local thumbnail = UserThumbnail.new(self.appState, user.id, false)
	thumbnail.rbx.Size = UDim2.new(0, THUMBNAIL_SIZE, 0, THUMBNAIL_SIZE)

	self.userThumbnail = Create.new"Frame" {
		Name = "UserThumbnail",
		BackgroundTransparency = 1,
