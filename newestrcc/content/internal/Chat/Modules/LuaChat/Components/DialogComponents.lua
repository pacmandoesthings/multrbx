...
local function addCancelButtonCallbacks(dialogComponent, cancelButton, event)
	...
	cancelButton.MouseButton1Click:connect(function()
		dialogComponent.rbx.Visible = false
		dialogComponent.appState.store:Dispatch({
			type = ActionType.PopRoute,
		})
		if event ~= nil then
			event:Fire()
...
local function ResizeFrame(frame)
	...
	local sizeFrame = UDim2.new(1, -Constants.ModalDialog.CLEARANCE_DIALOG_SIDE, 0, verticalSize)
...
function ConfirmationDialog:Prompt()
	self.rbx.Visible = true
end
...
function TextInputDialog:Close()
	self.rbx.Visible = false
	self.textInputComponent.textBoxComponent:ReleaseFocus()
end

function TextInputDialog:Prompt()
	self.rbx.Visible = true
end
...
function OptionDialog.new(appState, titleKey, options)
	local self = {
		appState = appState,
	}
	...
	self.rbx.Dialog.Size = UDim2.new(1, -48, 0, 0)
	...
	local firstDivider = makeDivider(10, Constants.Color.WHITE, 1, 1)
	...
	local cancelButtonFrame = Create.new"ImageLabel" {
		Name = "CancelButton",
		BackgroundTransparency = 1,
		Size = UDim2.new(1, -Constants.ModalDialog.CLEARANCE_DIALOG_SIDE,
			0, Constants.ModalDialog.BUTTON_HEIGHT),
		BorderSizePixel = 0,
		ScaleType = "Slice",
		SliceCenter = Rect.new(5,5,6,6),
		Image = "rbxasset://textures/ui/LuaChat/9-slice/modal.png",
	...
	local secondDivider = makeDivider(Constants.ModalDialog.CLEARANCE_DIALOG_BOTTOM, Constants.Color.WHITE, 1, 3)
...
function OptionDialog:Prompt(data)
	self.data = data
	self.rbx.Visible = true
end
