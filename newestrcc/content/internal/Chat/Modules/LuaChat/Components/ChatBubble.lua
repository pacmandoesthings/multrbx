...
function ChatBubble.new(appState, message)
	...
	if isFFlagEnabled("LuaChatAssetCards") then
		...
		elseif message.content:match("roblox%.com/[^%d]*(%d+)") then
			local linkStart, endLink

			local text = message.content

			for assetId in message.content:gmatch("roblox%.com/[^%d]*(%d+)") do

				linkStart, endLink = text:find("[^%s*]*roblox%.com/[^%s%d]*(%d+)[^%s*]*")
