...
local function CreateModeratedText(appState)
	return Create.new "Frame" {
		...
		Create.new "TextLabel" {
			...
			TextSize = 14,
			Text = appState.localization:Format(StringsLocale.Keys.THIS_MESSAGE_WAS_MODERATED),
			Font = Enum.Font.SourceSans,
...
function UserChatBubble:Update(message)
	...
	if message.moderated or message.failed then
		-- "There was an error and this message was not sent"
		if message.moderated and not self.moderatedText then
			self.moderatedText = CreateModeratedText(self.appState)
