	...
	if LayoutInfo.isLandscape then
		...
		if state ~= STATE.HIDDEN then
			if state == STATE.CLOSED then
				tween(topMenuContainer, tweenInfo, { Size = UDim2.new(0, 98, 0, 98) })
			elseif state == STATE.OPEN then
				tween(topMenuContainer, tweenInfo, { Size = UDim2.new(0, 98, 1, -22) })
			end
		end
	else
		local tweenTime = 0.1
		...
		if state == STATE.HIDDEN then
			tween(topMenuContainer, tweenInfo, { Position = UDim2.new(0, -300, 0, 0) })
		elseif state == STATE.CLOSED then
			tween(topMenuContainer, tweenInfo, { Position = UDim2.new(0, -300, 0, -10) })
		elseif state == STATE.OPEN then
			local openTweenInfo = TweenInfo.new(tweenTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0)
			tween(topMenuContainer, openTweenInfo, { Position = UDim2.new(0, -357 + #categories * 61, 0, -10) })
		end
	end
end
...
function this:openTopMenu()
	...
	if LayoutInfo.isLandscape then
		...
		tween(closeButtonImage, tweenInfo, { ImageTransparency = 0 }, { ImageTransparency = 1 })
	...
	for index, category in pairs(categories) do
		...
		if LayoutInfo.isLandscape then
			...
			categoryButton.TextLabel.Text = category.display or Strings:LocalizedString("ErrorDisplayTitle")
			...
			delay((index-1)/6*0.2, function()
				if categoryButton:IsDescendantOf(game) then
					...
					tween(categoryButton.CircleLabel, tweenInfo, imageGoals)
					tween(categoryButton.IconLabel, tweenInfo, imageGoals)
					tween(categoryButton.TextLabel, tweenInfo, textGoals)
				end
			end)
		else
...
function this:closeTopMenu()
	...
	if LayoutInfo.isLandscape then
		...
		if closeButton then
			...
			tween(closeButton, tweenInfo, propGoals).Completed:Connect(function()
				closeButton:Destroy()
			end)
		end
	end
