...
local EXTRA_VERTICAL_SHIFT = LayoutInfo.ExtraVerticalShift
...
function this:renderAssetCardByIndex(i)
	...
	local cardName, cardImage, clickFunction, isSelected
	...
	if currentPage.specialPageType == 'Outfits' or currentPage.specialPageType == 'Recent Outfits' then
		...
		local wearFunction = function()
			...
			if assetInfo then
				...
				if not assetTypeName:find('Animation') then
	...
	local card = CardGrid:makeAssetCard(i, cardName, cardImage, clickFunction, isSelected)
...
local function loadMoreListContent()
	...
	elseif currentPage.specialPageType == 'Skin Tone' then
		...
		--Implement UI
		equippedFrame.Image = "rbxasset://textures/ui/Shell/AvatarEditor/color selector/color dot-selected.png"
		equippedFrame.BackgroundTransparency = 1
		equippedFrame.AnchorPoint = Vector2.new(0.5, 0.5)
		equippedFrame.Size = UDim2.new(1, 10, 1, 10)
		equippedFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
		equippedFrame.ZIndex = LayoutInfo.EquippedFrameLayer

		local checkMark = Instance.new('ImageLabel')
		checkMark.Name = "CheckMark"
		checkMark.Image = "rbxasset://textures/ui/Shell/AvatarEditor/icon/ic-checkmark.png"
		checkMark.BackgroundTransparency = 1
		checkMark.AnchorPoint = Vector2.new(0.5, 0.5)
		checkMark.Size = UDim2.new(0, 32, 0, 32)
		checkMark.Position = UDim2.new(0.5, 0, 0.5, 0)
		checkMark.ZIndex = LayoutInfo.EquippedFrameLayer
		checkMark.Parent = equippedFrame

		colorSelector = Instance.new('ImageLabel')
		colorSelector.Name = "Selector"
		colorSelector.Image = "rbxasset://textures/ui/Shell/AvatarEditor/color selector/color dot-select.png"
		colorSelector.BackgroundTransparency = 1
		colorSelector.Size = UDim2.new(1, 24, 1, 24)
		colorSelector.Position = UDim2.new(0, -12, 0, -12)
		colorSelector.ZIndex = LayoutInfo.EquippedFrameLayer
		...
		for i,brickColor in pairs(skinColorList) do
			...
			colorButton.Image = "rbxasset://textures/ui/Shell/AvatarEditor/color selector/color dot.png"
			colorButton.ImageColor3 = brickColor.Color
			colorButton.Size = UDim2.new(0, SKIN_COLOR_BUTTONS_SIZE, 0, SKIN_COLOR_BUTTONS_SIZE)
			colorButton.Position = UDim2.new(
				0,
				SKIN_COLOR_GRID_PADDING + (column-1)*(SKIN_COLOR_BUTTONS_SIZE + SKIN_COLOR_GRID_PADDING),
				0,
				SKIN_COLOR_GRID_PADDING + (row-1)*(SKIN_COLOR_BUTTONS_SIZE + SKIN_COLOR_GRID_PADDING) + SKIN_COLOR_EXTRA_VERTICAL_SHIFT
			)
			...
			colorButton.SelectionImageObject = colorSelector
			local colorButtonShadow = Instance.new('ImageLabel')
			colorButtonShadow.Name = 'DropShadow'
			colorButtonShadow.Image = "rbxasset://textures/ui/Shell/AvatarEditor/color selector/color dot shadow.png"
			colorButtonShadow.BackgroundTransparency = 1
			colorButtonShadow.ZIndex = colorButton.ZIndex - 1
			colorButtonShadow.Size = UDim2.new(1, 8, 1, 9)
			colorButtonShadow.AnchorPoint = Vector2.new(0.5, 0.5)
			colorButtonShadow.Position = UDim2.new(0.5, 0, 0.5, 1)
	...
	if thisLoadingContentCall == currentLoadingContentCall then
		if not currentPage.special then
			...
			if #assetList == 0 and
				...
				noAssetsLabel.Name = "NoAssetsLabel"
				noAssetsLabel.BackgroundTransparency = 1
				noAssetsLabel.TextColor3 = LayoutInfo.WhiteTextColor
				noAssetsLabel.Size = UDim2.new(1,0,1,-SELECTOR_BOTTOM_MIN_DISTANCE)
				noAssetsLabel.Position = UDim2.new(0,0,0,0)
				noAssetsLabel.Font = LayoutInfo.RegularFont
				noAssetsLabel.TextSize = LayoutInfo.ButtonFontSize
				...
				if currentPage.specialPageType and string.find(currentPage.specialPageType, 'Recent') then
					noAssetsLabel.Text = string.format(Strings:LocalizedString("NoAssetsPhrase"), Strings:LocalizedString("RecentItemsWord"))
				else
		...
		Utilities.fastSpawn(function()
			if reachedBottom
				...
				recommendedLabel.Name = "recommendedTextLabel"
				...
				recommendedLabel.TextColor3 = Color3.new(1, 1, 1)
				recommendedLabel.Font = LayoutInfo.RegularFont
				recommendedLabel.Size = UDim2.new(1, 0, 0, 24)
				recommendedLabel.TextSize = LayoutInfo.SubHeaderFontSize
				recommendedLabel.TextXAlignment = "Left"
				recommendedLabel.ZIndex = LayoutInfo.AssetImageLayer
				...
				if recommendedAssetListRequest
					...
					local currRow = math.ceil(#assetList / BUTTONS_PER_ROW)
					...
					if recommendedCount > 0 then
						...
						scrollingFrame.CanvasSize = UDim2.new(0, 0, 0,
							...
							+ SELECTOR_BOTTOM_MIN_DISTANCE
							+ recommendedYOffset)
						if scrollingFrame:FindFirstChild("NoAssetsLabel") then
							scrollingFrame:FindFirstChild("NoAssetsLabel"):Destroy()
						end
					end
				end
			end
...
local function handleCameraRequest()
	local cFrame = LayoutInfo.CameraDefaultCFrame

	if currentPage.CameraPositionOffset then
		cFrame = cFrame * CFrame.new(currentPage.CameraPositionOffset)
	end

	if currentPage.CameraCFrameOffset then
		cFrame = cFrame * currentPage.CameraCFrameOffset
	end

	local position = cFrame.p
	local cameraFocus = currentPage.CameraFocus or {Parts = {'HumanoidRootPart'}}
	local focusPoint = characterManager.getFocusPoint(cameraFocus.Parts)

	updateCamera(position, focusPoint, currentPage.CameraFOV or 70)
end
	...
	if desiredPage ~= currentPage then
		...
		Utilities.fastSpawn(function()
			if desiredPage.typeName and desiredPage.typeName:find('Animation') then
				characterManager.startEquippedAnimationPreview(desiredPage.typeName)
			elseif currentPage == nil or currentPage.typeName and string.find(currentPage.typeName, 'Animation') then
				characterManager.startEquippedAnimationPreview('IdleAnimation')
			end
		end)
...
return function(inUserId, equippedFrameTemplate, inScrollingFrame, inCharacterManager)
