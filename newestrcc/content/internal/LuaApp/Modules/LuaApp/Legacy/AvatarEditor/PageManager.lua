...
function this:loadPage(assetTypeId, cursor)
...
local AvatarEditorUsesBrowserWindowCall = settings():GetFFlag("AvatarEditorUsesBrowserWindowCall")
...
local function loadMoreListContent()
	...
	if currentPage.typeName then
		...
		local pageInfo = this:loadPage(assetTypeId, nextCursor)
	...
	if thisLoadingContentCall == currentLoadingContentCall then
		...
		utilities.fastSpawn(function()
			if reachedBottom
				...
				local shopNowButtonTemplate = utilities.create'ImageButton'
				{
				...
				utilities.create'TextLabel'
				{
					...
					BackgroundTransparency = 1;
					BorderSizePixel = 0;
					Parent = shopNowButtonTemplate;
				}

				if shopNowButtonTemplate and AvatarEditorUsesBrowserWindowCall then
...
local function selectPage(desiredPage)
	if desiredPage ~= currentPage then
		...
		--mainFrame.BackgroundColor3 = desiredPage.name == 'Skin Tone'
		-- and Color3.fromRGB(255,255,255) or Color3.fromRGB(227,227,227)
...
local function previewPage(categoryIndex, tabIndex)
	local category = categories[categoryIndex]
	for _, page in next, category.pages do
		if page.typeName then
			this:loadPage(assetTypeNames[page.typeName], '')
		end
	end
	-- Check again in case CategoryIndex or TabIndex has already been changed
	local currentCategory = AppState.Store:GetState().Category
	local currentTabInfo = currentCategory.CategoryIndex and currentCategory.TabsInfo[currentCategory.CategoryIndex]
	local currentTabIndex = currentTabInfo and currentTabInfo.TabIndex or 1
	if currentCategory.CategoryIndex == categoryIndex and currentTabIndex == tabIndex then
		selectPage(category.pages[tabIndex])
	end
end
	...
	if newState.Category.CategoryIndex ~= oldState.Category.CategoryIndex or
		...
		utilities.fastSpawn(function()
			if newState.Category.CategoryIndex ~= oldState.Category.CategoryIndex then
				--Change category
				previewPage(categoryIndex, tabIndex)
			else
				--Change tab
				selectPage(categories[categoryIndex].pages[tabIndex])
			end
		end)
	end
end
