...
local Constants = require(coreGui.RobloxGui.Modules.Mobile.Constants)
...
local guiRoot = Create"Frame"
{
	...
	Size = UDim2.new(1,0,1,-Constants.Header.HEIGHT);
...
function adjustCameraProperties(instant)
	...
	if flagManager.AvatarEditorCameraZoomingEnabled then
		...
		do -- pixel perfect camera rotation
			...
			if viewMode then
				centerPixel = camera.ViewportSize * Vector2.new(0.5, 0.5) + Vector2.new(0, -Constants.Header.HEIGHT)
			elseif tabletMode then
				centerPixel = camera.ViewportSize * Vector2.new(0.3, 0.5) + Vector2.new(-45, 0)
			elseif phoneMode then
				centerPixel = camera.ViewportSize * Vector2.new(0.5, 0.75) + Vector2.new(0, -Constants.Header.HEIGHT+18)
			end
