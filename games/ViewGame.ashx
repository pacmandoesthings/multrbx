...
$ServerFetch = $MainDB->prepare("SELECT SUM(playerCount) AS totalPlayerCount FROM (SELECT playerCount FROM open_servers WHERE gameID = :pid AND playerCount > 0) AS subquery");
$ServerFetch->execute([":pid" => $GameId]);
$Results2 = $ServerFetch->fetch(PDO::FETCH_ASSOC);

if ($Results2 && isset($Results2['totalPlayerCount'])) {
    $GamePlayed = intval($Results2['totalPlayerCount']);
}
...
switch(true){
	...
	default:
		$GameOptions = null;
		$Notice = null;
		switch(true){case($GameApproval !== "1"):die(header('Location: '. $baseUrl .'/IDE/Error.ashx?err=404'));break;}
		switch(true){case($GamePublic !== "1"):die(header('Location: '. $baseUrl .'/IDE/Error.ashx?err=404'));break;}
		break;
}
			   ...
			   <div id="RepositionBody">
				  ...
				  <?php echo $Notice; ?>
					 <div id="ItemContainer" class="PlaceItemContainer new-game-page">
							...
							switch(true){
								case ($RBXTICKET == null):
									echo "<div id='JoinRoblox' class='Ads_WideSkyscraper' style='float: left; height: 863px; width: 160px; border: 1px solid rgb(136, 136, 136); text-align: center; margin: 0px 8px 10px 0px;'><div style='padding:20px 7px;'><div style='font:normal 18px Arial, Helvetica, Sans-Serif; color:#666'>Join MULTRBX</div><hr class='PlaceItemHR'><div style='text-align:left; font-size:13px;'><div style='font-weight:bold;margin-top:15px;'>By joining MULTRBX, you can:</div><ul style='padding-left:15px;line-height:1.5em;'><li>Make friends</li><li>Change your character</li><li>Make your own game</li><li>Join groups</li><li>Use gear items</li></ul></div><div style='margin-top:65px; text-align:center'><a href='". $baseUrl ."/Login/loginmobile.php'><div class='btn-neutral btn-large'>Sign Up</div></a></div></div></div>";
									break;
							}
							echo "
...
// Check if the user agent contains "Roblox" or matches the specified Roblox Android App user agent
if (strpos($userAgent, 'Roblox') !== false || strpos($userAgent, 'WinInet') !== false) {
    $url = 'http://yourdomain.com/games/start?placeid=' . $GameId; // Replace 'yourdomain.com' with your actual domain
} else {
    $url = 'http://mulrbx.com/playgame.php?id=' . $GameId;
}
									...
									if ($RBXTICKET != NULL) {
										...
										echo "
									   <div id='ctl00_cphRoblox_NewGamePageControl_VisitButtons_MultiplayerVisitButton' class='VisitButton VisitButtonPlay' placeid='<?php echo $GameId; ?>'>
    <a href='". $url. "' style='color: white;' class='btn-medium btn-primary'>Play</a>
          </div>
											
									";									
									}
									echo "<a href='games/start?placeid=".$GameId."' class='btn-medium btn-primary' style='color: white;'>Play Mobile</a>";
								echo "
	<div>
